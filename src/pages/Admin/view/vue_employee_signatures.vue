...
async function submitForm() {
  ...
  try {
    ...
    if (editMode.value && editId.value) {
      await store.updateEmployee(editId.value, payload)
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await store.deleteEmployee(deleteTarget.value.id, deleteTarget.value.signature_image)
    store.fetchEmployees()
    closeDeleteConfirm()
...
const filteredList = computed(() => {
  if (!searchQuery.value.trim()) return store.employees
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <h1 class="page-title">จัดการลายเซ็นพนักงาน</h1>
          <p class="page-subtitle">บันทึกข้อมูลและรูปภาพลายเซ็นของพนักงาน</p>
        </div>
      </div>
      ...
      <div class="card-toolbar">
        ...
        <div class="toolbar-right">
          <div class="count-chip">
            <span>ทั้งหมด <strong>{{ store.employees.length }}</strong> รายการ</span>
          </div>
        </div>
      </div>
        ...
        <table class="doc-table">
          ...
          <tbody>
            ...
            <tr v-else-if="filteredList.length === 0">
              <td colspan="7" class="state-cell">ไม่พบข้อมูลพนักงาน</td>
            </tr>
      ...
      <transition name="slide">
        <div v-if="drawerOpen" class="drawer-panel">
          <div class="drawer-header">
            <h3>{{ editMode ? 'แก้ไขข้อมูลพนักงาน' : 'เพิ่มพนักงานใหม่' }}</h3>
            <button @click="closeDrawer"><i class="fa fa-times"></i></button>
          </div>
          <div class="drawer-body">
            <div class="form-group">
              <label>ชื่อ-นามสกุล <span class="text-red-500">*</span></label>
              <input v-model="form.full_name" type="text" class="form-input" placeholder="ระบุชื่อพนักงาน..." />
            </div>
            <div class="form-group">
      ...
      <transition name="fade">
        <div v-if="deleteConfirmOpen" class="modal-overlay" @click="closeDeleteConfirm">
          <div class="modal-box" @click.stop>
            ...
            <h3>ยืนยันการลบ</h3>
            <p>คุณต้องการลบพนักงาน <strong>{{ deleteTarget?.full_name }}</strong> ใช่หรือไม่?</p>
