...
const filteredList = computed(() => {
  ...
  return store.requests.filter(item =>
    ...
    (item.welfare_type || '').toLowerCase().includes(q) ||
    (item.welfare_item || '').toLowerCase().includes(q)
  )
})
...
const toBase64 = async (url) => {
  try {
    if (!url) return ''
    // Add cache-busting and crossorigin handling
    ...
    console.error('Base64 error for:', url, err)
    return '' 
  }
}
...
async function downloadPDF(item) {
  ...
  try {
    ...
    const [logo1, assetImgB64, sig1B64, sig2B64] = await Promise.all([
      toBase64(LOGO_URL_1),
    ...
    const htmlContent = buildPrintHTML(item, logo1, assetImgB64, sig1B64, sig2B64)
    ...
    iframe.onload = async () => {
      try {
        await new Promise(r => setTimeout(r, 1500)) // Wait for fonts and images
        ...
        const fileName = `ใบเบิกสวัสดิการ_${item.employee_code || 'Doc'}_${new Date().getTime()}.pdf`
        pdf.save(fileName)
...
function buildPrintHTML(item, logo1, assetImg, sig1, sig2) {
      ...
      <body>
        ...
        <div class="a4-sheet">
          <div class="doc-header">
            <div class="doc-logo"><img src="${logo1}" crossorigin="anonymous"></div>
          ...
          <div class="sig-bottom-wrap">
            <div class="sig-col">
              ...
              ${sig1 ? `<div class="sig-col-img-wrap"><img src="${sig1}" class="sig-col-img" crossorigin="anonymous"></div>` : '<div class="sig-col-dots">.................................</div>'}
            ...
            <div class="sig-col">
              <div class="sig-col-label">ลงชื่อ:</div>
              ${sig2 ? `<div class="sig-col-img-wrap"><img src="${sig2}" class="sig-col-img" crossorigin="anonymous"></div>` : '<div class="sig-col-dots">.................................</div>'}
        ...
        <div class="a4-sheet">
          ...
          <div class="p2-photo-wrap">
            ${assetImg ? `<img src="${assetImg}" crossorigin="anonymous">` : '<div style="color:#ccc; font-size:14pt;">📷</div>'}
          </div>
...
async function handleDelete(item) {
  const result = await Swal.fire({
    title: 'ยืนยันการลบ?',
    html: `ลบข้อมูลขอเบิกสวัสดิการของ <strong>${item.first_name} ${item.last_name}</strong><br>
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        <div class="page-icon-wrap"><i class="fas fa-hand-holding-heart"></i></div>
        <div>
          <h1 class="page-title">รายงานข้อมูลสวัสดิการพนักงาน</h1>
          <p class="page-subtitle">แสดงรายการประวัติการขอเบิกสวัสดิการทั้งหมดในระบบ</p>
        </div>
      </div>
      <div class="page-header-right">
        <button class="btn-back" @click="router.push('/admin/welfare-request')">
            ...
            <tr v-for="(item, idx) in filteredList" :key="item.id" class="data-row">
              ...
              <td>
                <div class="img-preview-cell" v-if="item.image_url" @click="openImagePreview(item.image_url)" style="cursor: zoom-in;" title="คลิกเพื่อดูรูปขยาย">
                  <img :src="item.image_url" alt="Welfare" />
                </div>
                <span v-else class="null-dash">ไม่มีรูป</span>
              </td>
              <td>
                <span class="item-type">{{ item.welfare_type || '-' }}</span>
              </td>
              <td>
                <div class="item-name">{{ item.welfare_item || '-' }}</div>
              </td>
              <td>
                <div class="cell-name">
                  <span class="name-text">{{ item.title || '' }}{{ item.first_name }} {{ item.last_name }}</span>
                  <span class="badge-code" v-if="item.employee_code"> / {{ item.employee_code }}</span>
                </div>
              </td>
              <td>
                <div class="date-cell">
              ...
              <td>
                <div class="action-wrap">
                  <button class="btn-pdf" @click="downloadPDF(item)"
                    ...
                    {{ downloadingId === item.id ? 'กำลังสร้าง...' : 'พิมพ์เอกสาร' }}
                  </button>
...
.page-icon-wrap{width:44px;height:44px;border-radius:12px;background:#10b981;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
.result-chip{display:inline-flex;align-items:center;gap:6px;padding:5px 12px;background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.18);border-radius:20px;font-size:12px;font-weight:600;color:#10b981;white-space:nowrap;}
...
.creator-col {display:inline-flex;align-items:center;gap:5px;padding:3px 8px;background:#f1f5f9;border-radius:6px;color:#475569;font-weight:700;font-size:11.5px;}
.cell-name{display:flex;align-items:center;gap:10px;}
.avatar-circle{width:28px;height:28px;border-radius:50%;background:#10b981;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;text-transform:uppercase;}
.name-text{font-weight:700;color:#1e293b;}

.item-info { display: flex; flex-direction: column; gap: 2px; }
.item-type { font-weight: 700; color: #10b981; font-size: 11px; }
.item-name { color: #475569; font-weight: 600; }

.amount-text { font-weight: 800; color: #059669; }
...
.btn-pdf{display:inline-flex;align-items:center;gap:6px;padding:6px 12px;background:#10b981;border:none;border-radius:8px;color:#fff;font-size:11.5px;font-weight:700;cursor:pointer;transition:all 0.2s;}
.btn-pdf:hover:not(:disabled){background:#059669;transform:translateY(-1px);box-shadow:0 4px 6px -1px rgba(16,185,129,0.2);}
.btn-pdf:disabled{opacity:0.6;cursor:not-allowed;}

.btn-edit-action{width:30px;height:30px;display:flex;align-items:center;justify-content:center;background:#fff;border:1.5px solid #e2e8f0;border-radius:8px;color:#10b981;cursor:pointer;transition:all 0.2s;}
.btn-edit-action:hover{background:#10b981;border-color:#10b981;color:#fff;}
