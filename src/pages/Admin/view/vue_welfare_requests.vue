...
const store = useWelfareTypeStore()
...
async function submitForm() {
  ...
  try {
    if (editMode.value && editId.value) {
      await store.updateType(editId.value, {
...
async function confirmDelete() {
  ...
  deleteError.value = ''
  try {
    await store.deleteType(deleteTarget.value.id)
    store.fetchTypes()
    closeDeleteConfirm()
    Swal.fire({ title: 'สำเร็จ!', text: 'ลบข้อมูลแล้ว', icon: 'success', timer: 1500, showConfirmButton: false })
  } catch (e) {
    Swal.fire({ title: 'เกิดข้อผิดพลาด', text: e.message, icon: 'error' })
  } finally {
...
const filteredList = computed(() => {
  if (!searchQuery.value.trim()) return store.types
  const q = searchQuery.value.toLowerCase()
  return store.types.filter(item =>
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <h1 class="page-title">หัวข้อสวัสดิการพนักงาน</h1>
          <p class="page-subtitle">จัดการรายการหัวข้อสำหรับการเบิกสวัสดิการ</p>
        </div>
      </div>
      ...
      <div class="card-toolbar">
        ...
        <div class="toolbar-right">
          <div class="count-chip">
            <span>ทั้งหมด <strong>{{ store.types.length }}</strong> รายการ</span>
          </div>
        </div>
      </div>
        ...
        <div v-if="drawerOpen" class="drawer-panel">
          ...
          <div class="drawer-body">
            <div class="form-group">
              <label>หัวข้อสวัสดิการ <span class="text-red-500">*</span></label>
              <input v-model="form.title" type="text" class="form-input" placeholder="ระบุหัวข้อ..." @keyup.enter="submitForm" />
            </div>
          </div>
...
.page-icon-wrap { width: 45px; height: 45px; background: #10b981; color: white; display: flex; align-items: center; justify-content: center; border-radius: 10px; font-size: 20px; }
.page-title { font-size: 20px; font-weight: 800; margin: 0; }
.page-subtitle { font-size: 13px; color: #666; margin: 0; }

.btn-add { background: #10b981; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px; }
...
.doc-table td { padding: 12px; border-bottom: 1px solid #eee; font-size: 13px; }
...
.btn-edit { color: #0ea5e9; background: rgba(14, 165, 233, 0.1); }
...
.btn-save { background: #10b981; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 700; cursor: pointer; }
...
.btn-delete-confirm { background: #ef4444; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 700; cursor: pointer; }
