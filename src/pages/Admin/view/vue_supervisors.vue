...
function openDrawer() {
  ...
  editId.value = null
  form.value = { full_name: '', position: '', signature_image: '' }
...
function openEditDrawer(item) {
  ...
  imagePreview.value = item.signature_image || ''
  selectedFile.value = null
  drawerOpen.value = true
}
...
async function submitForm() {
  if (!form.value.full_name.trim() || !form.value.position.trim()) {
    Swal.fire({ title: 'ข้อมูลไม่ครบ', text: 'กรุณากรอกชื่อและตำแหน่งให้ครบถ้วน', icon: 'warning' })
    return
  }
  
  saving.value = true
  try {
    let imageUrl = form.value.signature_image

    // Upload new image if selected
    if (selectedFile.value) {
      imageUrl = await store.uploadSignature(selectedFile.value)
    }
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await store.deleteSupervisor(deleteTarget.value.id, deleteTarget.value.signature_image)
    store.fetchSupervisors()
    closeDeleteConfirm()
...
// ── Table ─────────────────────────────────────────────────
onMounted(() => store.fetchSupervisors())
...
const filteredList = computed(() => {
  if (!searchQuery.value.trim()) return store.supervisors
  const q = searchQuery.value.toLowerCase()
  return store.supervisors.filter(item =>
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <h1 class="page-title">จัดการลายเซ็นหัวหน้า</h1>
          <p class="page-subtitle">บันทึกข้อมูลและรูปภาพลายเซ็นของหัวหน้างาน</p>
        </div>
      </div>
      <div class="page-header-right">
        <button class="btn-add" @click="openDrawer">
          <i class="fa fa-plus"></i>
          <span>เพิ่มหัวหน้าใหม่</span>
        </button>
      </div>
    </div>
      ...
      <div class="card-toolbar">
        ...
        <div class="toolbar-right">
          <div class="count-chip">
            <span>ทั้งหมด <strong>{{ store.supervisors.length }}</strong> รายการ</span>
          </div>
        </div>
      </div>
        ...
        <table class="doc-table">
          ...
          <tbody>
            <tr v-if="store.loading">
              <td colspan="7" class="state-cell">กำลังโหลดข้อมูล...</td>
            </tr>
            <tr v-else-if="filteredList.length === 0">
              <td colspan="7" class="state-cell">ไม่พบข้อมูลหัวหน้างาน</td>
            </tr>
            <tr v-for="(item, idx) in filteredList" :key="item.id">
              <td class="td-num">{{ idx + 1 }}</td>
              <td>
                <div class="signature-preview-cell" v-if="item.signature_image">
                  <img :src="item.signature_image" alt="Signature" />
                </div>
      ...
      <transition name="slide">
        <div v-if="drawerOpen" class="drawer-panel">
          <div class="drawer-header">
            <h3>{{ editMode ? 'แก้ไขข้อมูลหัวหน้า' : 'เพิ่มหัวหน้าใหม่' }}</h3>
            <button @click="closeDrawer"><i class="fa fa-times"></i></button>
          </div>
          <div class="drawer-body">
            <div class="form-group">
              <label>ชื่อ-นามสกุล <span class="text-red-500">*</span></label>
              <input v-model="form.full_name" type="text" class="form-input" placeholder="ระบุชื่อหัวหน้า..." />
            </div>
            <div class="form-group">
              <label>ตำแหน่ง <span class="text-red-500">*</span></label>
              <input v-model="form.position" type="text" class="form-input" placeholder="ระบุตำแหน่ง..." />
            </div>
            <div class="form-group">
              ...
              <div class="upload-area" @click="$refs.fileInput.click()">
                <div v-if="imagePreview" class="preview-wrap">
                  ...
                  <div class="upload-overlay">เปลี่ยนรูปภาพ</div>
                </div>
                <div v-else class="upload-placeholder">
                  <i class="fa fa-cloud-arrow-up text-3xl mb-2"></i>
                  <span>คลิกเพื่ออัปโหลดรูปภาพ</span>
                </div>
                <input type="file" ref="fileInput" class="hidden" accept="image/*" @change="handleFileChange" />
              </div>
              <p class="text-xs text-gray-500 mt-2">รองรับไฟล์ภาพ JPG, PNG, WEBP</p>
            </div>
          </div>
      ...
      <transition name="fade">
        <div v-if="deleteConfirmOpen" class="modal-overlay" @click="closeDeleteConfirm">
          <div class="modal-box" @click.stop>
            ...
            <h3>ยืนยันการลบ</h3>
            <p>คุณต้องการลบหัวหน้า <strong>{{ deleteTarget?.full_name }}</strong> ใช่หรือไม่?</p>
...
.signature-preview-cell { width: 100px; height: 40px; border: 1px solid #eee; border-radius: 4px; overflow: hidden; background: #fcfcfc; }
.signature-preview-cell img { width: 100%; height: 100%; object-fit: contain; }
...
/* Upload Area */
.upload-area { border: 2px dashed #ddd; border-radius: 12px; height: 160px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; position: relative; overflow: hidden; }
...
.preview-wrap img { width: 100%; height: 100%; object-fit: contain; padding: 10px; }
.upload-overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.4); color: white; display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.2s; font-weight: 700; }
.preview-wrap:hover .upload-overlay { opacity: 1; }
