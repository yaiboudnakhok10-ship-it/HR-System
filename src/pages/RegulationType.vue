...
const store = useRegulationTypeStore()
...
function addListItem() {
  const val = newListItem.value.trim()
  if (!val) return
  form.value.regulation_list.push({ name: val })
  newListItem.value = ''
}

function removeListItem(idx) {
  form.value.regulation_list.splice(idx, 1)
}
...
function openEditDrawer(item) {
  ...
  form.value = {
    regulation_type_name: item.regulation_type_name || '',
    regulation_list: item.regulation_list ? JSON.parse(JSON.stringify(item.regulation_list)) : [],
  }
  drawerOpen.value = true
}

function resetForm() {
  form.value = { regulation_type_name: '', regulation_list: [] }
  newListItem.value = ''
...
async function submitForm() {
  if (!form.value.regulation_type_name) return
  saving.value = true
  try {
    if (editMode.value && editId.value) {
      await store.updateRegulationType(editId.value, {
        regulation_type_name: form.value.regulation_type_name,
        regulation_list:      form.value.regulation_list,
      })
    } else {
      await store.addRegulationType({
        regulation_type_name: form.value.regulation_type_name,
        regulation_list:      form.value.regulation_list,
...
async function confirmDelete() {
  ...
  deleteError.value = ''
  try {
    await store.deleteRegulationType(deleteTarget.value.id)
    store.getRegulationTypes()
    closeDeleteConfirm()
  } catch (e) {
    if (e.message === 'FK_VIOLATION') {
      ...
      deleteError.value = 'ເກີດຂໍ້ຜິດພາດ: ' + e.message
    }
  } finally {
...
// ── Table ─────────────────────────────────────────────────
onMounted(() => store.getRegulationTypes())
...
const filteredList = computed(() => {
  if (!searchQuery.value.trim()) return store.regulation_types
  const q = searchQuery.value.toLowerCase()
  return store.regulation_types.filter(item =>
    (item.regulation_type_name || '').toLowerCase().includes(q) ||
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <h1 class="page-title">ประเภทกฎระเบียบ</h1>
          <p class="page-subtitle">จัดการข้อมูลประเภทกฎระเบียบทั้งหมด</p>
        </div>
      </div>
      <div class="page-header-right">
        <button class="btn-add" @click="openDrawer">
          <i class="fa fa-plus"></i>
          <span>เพิ่มประเภทกฎระเบียบ</span>
        </button>
      </div>
    </div>
      ...
      <div class="card-toolbar">
        <div class="toolbar-left">
          <div class="search-box">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="ค้นหา ชื่อประเภท, ผู้เพิ่ม..."
              class="search-input"
            />
        ...
        <div class="toolbar-right">
          <div class="count-chip">
            ...
            <span>ทั้งหมด <strong>{{ store.regulation_types.length }}</strong> รายการ</span>
          </div>
        </div>
      </div>
      ...
      <div class="table-wrapper">
        <table class="doc-table">
          <thead>
            <tr>
              <th class="th-num">#</th>
              <th>ชื่อประเภทกฎระเบียบ</th>
              <th>รายการกฎระเบียบ</th>
              <th>ผู้เพิ่ม</th>
              <th>บันทึกเมื่อ</th>
            ...
            <tr v-else-if="filteredList.length === 0">
              <td colspan="6" class="state-cell">
                <div class="state-content empty">
                  ...
                  <span class="empty-title">{{ searchQuery ? 'ไม่พบข้อมูลที่ค้นหา' : 'ไม่พบข้อมูลประเภทกฎระเบียบ' }}</span>
                  ...
                  <button v-else class="btn-add-empty" @click="openDrawer">
                    <i class="fa fa-plus"></i> เพิ่มประเภทกฎระเบียบรายการแรก
                  </button>
                </div>
              </td>
            </tr>

            <tr v-else v-for="(item, idx) in filteredList" :key="item.id" class="data-row">
              ...
              <td>
                <div class="cell-name">
                  ...
                  <span class="name-text">{{ item.regulation_type_name || '—' }}</span>
                </div>
              </td>
              <td>
                <div class="list-tags" v-if="item.regulation_list && item.regulation_list.length">
                  <span
                    class="list-tag"
                    v-for="(li, i) in item.regulation_list.slice(0, 3)"
                    :key="i"
                  >{{ li.name }}</span>
                  <span class="list-tag-more" v-if="item.regulation_list.length > 3">
                    +{{ item.regulation_list.length - 3 }} รายการ
                  </span>
                </div>
      ...
      <div class="modal-overlay" v-if="deleteConfirmOpen" @click="closeDeleteConfirm">
        <transition name="modal-pop">
          <div class="modal-box" v-if="deleteConfirmOpen" @click.stop>
            ...
            <p class="modal-desc">
              คุณต้องการลบประเภทกฎระเบียบ<br/>
              <strong>{{ deleteTarget?.regulation_type_name }}</strong> ใช่หรือไม่?<br/>
      ...
      <div class="drawer-panel" v-if="drawerOpen">
        ...
        <div class="drawer-header">
          <div class="drawer-header-left">
            ...
            <div>
              <h2 class="drawer-title">{{ editMode ? 'แก้ไขประเภทกฎระเบียบ' : 'เพิ่มประเภทกฎระเบียบใหม่' }}</h2>
        ...
        <div class="drawer-body">
          ...
          <div class="form-section">
            <div class="form-section-label">
              <i class="fa fa-scale-balanced"></i> ข้อมูลประเภทกฎระเบียบ
            </div>
            <div class="form-group">
              <label class="form-label required">ชื่อประเภทกฎระเบียบ</label>
              <input
                v-model="form.regulation_type_name"
                type="text"
                class="form-input"
                placeholder="ระบุชื่อประเภทกฎระเบียบ..."
              />
            </div>
          </div>
          ...
          <div class="form-section">
            <div class="form-section-label">
              <i class="fa fa-list-ul"></i> รายการกฎระเบียบ
            </div>
            ...
            <div class="list-input-wrap">
              <input
                ...
                type="text"
                class="form-input"
                placeholder="พิมพ์รายการแล้วกด Enter หรือปุ่ม +"
                @keydown="handleListKeydown"
              />
              <button class="btn-list-add" @click="addListItem" :disabled="!newListItem.trim()">
                <i class="fa fa-plus"></i>
              </button>
            </div>

            <!-- รายการที่เพิ่มแล้ว -->
            <div class="list-items-wrap" v-if="form.regulation_list.length">
              <div
                class="list-item-row"
                v-for="(li, idx) in form.regulation_list"
                :key="idx"
              >
                ...
                <input
                  v-model="li.name"
                  type="text"
                  class="form-input list-item-input"
                  placeholder="ชื่อรายการ..."
                />
                <button class="btn-list-remove" @click="removeListItem(idx)">
                  <i class="fa fa-xmark"></i>
                </button>
              </div>
            </div>

            <div class="list-empty-hint" v-else>
              <i class="fa fa-circle-info"></i>
              ยังไม่มีรายการ กรุณาเพิ่มรายการกฎระเบียบ
            </div>
          </div>
        ...
        <div class="drawer-footer">
          ...
          <button
            class="btn-save"
            @click="submitForm"
            :disabled="!form.regulation_type_name || saving"
          >
...
.doc-table { width: 100%; border-collapse: collapse; font-size: 12.5px; min-width: 650px; }
...
.list-tag { display: inline-flex; align-items: center; padding: 2px 8px; background: rgba(25,118,210,0.08); border: 1px solid rgba(25,118,210,0.18); border-radius: 12px; font-size: 11px; font-weight: 600; color: var(--accent); white-space: nowrap; }
.list-tag-more { display: inline-flex; align-items: center; padding: 2px 8px; background: var(--sb-search-bg); border: 1px solid var(--tb-border); border-radius: 12px; font-size: 11px; font-weight: 600; color: var(--tb-sub); white-space: nowrap; }
...
.drawer-panel { position: fixed; top: 0; right: 0; bottom: 0; width: 440px; max-width: 100vw; background: var(--tb-bg); border-left: 1px solid var(--tb-border); box-shadow: -8px 0 40px rgba(0,0,0,0.14); z-index: 500; display: flex; flex-direction: column; overflow: hidden; }
...
/* List input */
.list-input-wrap { display: flex; gap: 8px; align-items: center; }
.list-input-wrap .form-input { flex: 1; }
.btn-list-add { width: 38px; height: 38px; border-radius: 8px; background: var(--accent); color: #fff; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 14px; flex-shrink: 0; transition: all 0.2s; box-shadow: 0 2px 8px var(--accent-glow); }
.btn-list-add:hover:not(:disabled) { background: var(--accent-dark); transform: translateY(-1px); }
.btn-list-add:disabled { opacity: 0.4; cursor: not-allowed; }

/* List items */
.list-items-wrap { display: flex; flex-direction: column; gap: 6px; max-height: 280px; overflow-y: auto; padding-right: 2px; }
.list-items-wrap::-webkit-scrollbar { width: 3px; }
.list-items-wrap::-webkit-scrollbar-thumb { background: var(--tb-border); border-radius: 3px; }
.list-item-row { display: flex; align-items: center; gap: 8px; }
.list-item-num { width: 22px; height: 22px; border-radius: 50%; background: var(--accent); color: #fff; font-size: 10px; font-weight: 800; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.list-item-input { flex: 1; padding: 7px 10px !important; font-size: 12.5px !important; }
.btn-list-remove { width: 28px; height: 28px; border-radius: 7px; background: rgba(239,68,68,0.08); color: #ef4444; border: 1px solid rgba(239,68,68,0.18); cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 11px; flex-shrink: 0; transition: all 0.18s; }
.btn-list-remove:hover { background: #ef4444; color: #fff; }

.list-empty-hint { display: flex; align-items: center; gap: 7px; padding: 10px 14px; background: var(--sb-search-bg); border: 1.5px dashed var(--tb-border); border-radius: 8px; font-size: 12px; font-weight: 600; color: var(--tb-sub); }
.list-empty-hint i { color: var(--accent); font-size: 13px; }
