...
function getDefaultRecorder() {
  return auth.session?.fullname || auth.session?.username || ''
}

const hiddenRecordedBy = ref(getDefaultRecorder())

function openDrawer() {
  editMode.value = false
  editId.value = null
  form.value = { topic: '' }
  hiddenRecordedBy.value = getDefaultRecorder()
...
function openEditDrawer(item) {
  ...
  hiddenRecordedBy.value = item.recorded_by || getDefaultRecorder()
  drawerOpen.value = true
}
...
async function submitForm() {
  ...
  const recorded_by = hiddenRecordedBy.value?.trim() || getDefaultRecorder()

  if (!topic) {
    await Swal.fire({
      icon: 'warning',
      title: 'กรอกข้อมูลไม่ครบ',
      text: 'กรุณาระบุ "ประเภทการเตือน" ให้ครบถ้วน',
  ...
  try {
    if (editMode.value && editId.value) {
      ...
      await store.addTopicRecord({ topic, recorded_by })
      await Swal.fire({
        icon: 'success',
...
async function confirmDelete(item) {
  const result = await Swal.fire({
    icon: 'warning',
    title: 'ยืนยันการลบ',
    html: `ต้องการลบประเภทการเตือน<br/><strong>${item?.topic || ''}</strong> ใช่หรือไม่?`,
  ...
  try {
    await store.deleteTopicRecord(item.id)
    store.getTopicRecords()
    await Swal.fire({
      icon: 'success',
      title: 'ลบแล้ว',
    ...
    const msg =
      e.message === 'FK_VIOLATION'
        ? 'ไม่สามารถลบได้ เนื่องจากข้อมูลนี้ถูกใช้งานอยู่ในระบบ'
        : e.message || 'ไม่สามารถลบข้อมูลได้'
    await Swal.fire({
      icon: 'error',
      title: 'ลบไม่สำเร็จ',
      text: msg,
...
onMounted(() => store.getTopicRecords())

const filteredList = computed(() => {
  if (!searchQuery.value.trim()) return store.topic_records
  const q = searchQuery.value.toLowerCase()
  return store.topic_records.filter((item) => {
    return (
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          ...
          <p class="page-subtitle">จัดการข้อมูลประเภทการเตือน (เพิ่ม/แก้ไข/ลบ)</p>
        </div>
      </div>
        ...
        <div class="toolbar-left">
          <div class="search-box">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="ค้นหา ประเภทการเตือน, ผู้บันทึก..."
              class="search-input"
            />
              ...
              <td colspan="5" class="state-cell">
                <div class="state-content empty">
                  ...
                  <button v-else class="btn-add-empty" @click="openDrawer">
                    <i class="fa fa-plus"></i> เพิ่มรายการแรก
                  </button>
                </div>
              </td>
            </tr>
            ...
            <tr v-else v-for="(item, idx) in filteredList" :key="item.id" class="data-row">
              ...
              <td>
                <div class="cell-name">
                  ...
                  <span class="name-text">{{ item.topic || '—' }}</span>
                </div>
              </td>
              <td>
                <span class="creator-tag" v-if="item.recorded_by">
                  <i class="fa fa-circle-user"></i>{{ item.recorded_by }}
                </span>
              ...
              <td>
                <div class="date-cell">
                  ...
                  <span>{{ formatDate(item.recorded_at) }}</span>
                </div>
              </td>
              <td class="td-actions">
                <div class="actions-wrap">
                  ...
                  <button class="btn-action btn-delete" @click="confirmDelete(item)" title="ลบ">
      ...
      <div class="drawer-panel" v-if="drawerOpen">
        <div class="drawer-header">
          <div class="drawer-header-left">
            ...
            <div>
              <h2 class="drawer-title">{{ editMode ? 'แก้ไขประเภทการเตือน' : 'เพิ่มประเภทการเตือน' }}</h2>
              <p class="drawer-sub">{{ editMode ? 'แก้ไขข้อมูลแล้วกดบันทึก' : 'กรอกข้อมูลแล้วกดบันทึก' }}</p>
            </div>
          </div>
        ...
        <div class="drawer-body">
          <div class="form-section">
            <div class="form-section-label">
              <i class="fa fa-scale-balanced"></i> ข้อมูลประเภทการเตือน
            </div>
            <div class="form-group">
              <label class="form-label required">ประเภทการเตือน</label>
              <input
                v-model="form.topic"
                type="text"
                class="form-input"
                placeholder="ระบุประเภทการเตือน..."
              />
            </div>
          </div>
        </div>
