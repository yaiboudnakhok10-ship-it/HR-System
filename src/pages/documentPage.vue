<script setup>
import { document } from '../stores/document.stores'
...
function resetForm() {
  form.value = { document_name: '', detail: '' }
  editMode.value = false
  editId.value   = null
}

async function submitForm() {
  if (!form.value.document_name) return
  saving.value = true
  try {
    if (editMode.value && editId.value) {
      await store.updateDocument(editId.value, form.value)
    } else {
      // ✅ เพิ่ม created_by และ created_at
...
const filteredDocuments = computed(() => {
  if (!searchQuery.value.trim()) return store.document_type
  const q = searchQuery.value.toLowerCase()
  return store.document_type.filter(item =>
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <h1 class="page-title">ประเภทลายเช็นหัวหน้างาน</h1>
          <p class="page-subtitle">จัดการข้อมูลประเภทลายเช็นของหัวหน้าทั้งหมด</p>
        </div>
      </div>
      <div class="page-header-right">
        <button class="btn-add" @click="openDrawer">
          <i class="fa fa-plus"></i>
          <span>เพิ่มประเภทลายเช็น</span>
        </button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-toolbar">
        <div class="toolbar-left">
          ...
          <transition name="fade">
            <div class="result-chip" v-if="searchQuery">
              ...
              พบ <strong>{{ filteredDocuments.length }}</strong> รายการ
            </div>
          </transition>
        </div>
        <div class="toolbar-right">
          <div class="count-chip">
            ...
            <span>ทั้งหมด <strong>{{ store.document_type.length }}</strong> รายการ</span>
          </div>
        </div>
      </div>
            ...
            <tr v-else-if="filteredDocuments.length === 0">
              <td colspan="6" class="state-cell">
                <div class="state-content empty">
                  ...
                  <span class="empty-title">{{ searchQuery ? 'ไม่พบข้อมูลที่ค้นหา' : 'ไม่พบข้อมูลประเภทลายเช็น' }}</span>
                  ...
                  <button v-else class="btn-add-empty" @click="openDrawer">
                    <i class="fa fa-plus"></i> เพิ่มประเภทลายเช็นรายการแรก
                  </button>
                </div>
              </td>
            </tr>

            <tr v-else v-for="(item, idx) in filteredDocuments" :key="item.id" class="data-row">
              ...
              <td>
                <div class="cell-name">
                  ...
                  <span class="name-text">{{ item.document_name || '—' }}</span>
                </div>
              </td>
      ...
      <!-- Footer -->
      <div class="card-footer" v-if="!store.loading && filteredDocuments.length > 0">
      ...
      <div class="modal-overlay" v-if="deleteConfirmOpen" @click="closeDeleteConfirm">
        <transition name="modal-pop">
          <div class="modal-box" v-if="deleteConfirmOpen" @click.stop>
            ...
            <p class="modal-desc">
              คุณต้องการลบประเภทลายเช็น<br/>
              <strong>{{ deleteTarget?.document_name }}</strong> ใช่หรือไม่?<br/>
      ...
      <div class="drawer-panel" v-if="drawerOpen">
        ...
        <div class="drawer-header">
          <div class="drawer-header-left">
            ...
            <div>
              <h2 class="drawer-title">{{ editMode ? 'แก้ไขประเภทลายเช็น' : 'เพิ่มประเภทลายเช็นใหม่' }}</h2>
        ...
        <div class="drawer-body">
          ...
          <div class="form-section">
            <div class="form-section-label">
              <i class="fa fa-file-lines"></i> ข้อมูลประเภทลายเช็น
            </div>
            <div class="form-group">
              <label class="form-label required">ชื่อประเภทลายเช็น</label>
              <input
                v-model="form.document_name"
                type="text"
                class="form-input"
                placeholder="ระบุชื่อประเภทลายเช็น..."
              />
            </div>
          </div>
          ...
          <div class="form-section">
            <div class="form-section-label">
              <i class="fa fa-note-sticky"></i> หน้าที่
            </div>
            <input type="text"  v-model="form.detail" class="form-input" placeholder="ระบุหน้าที่">
          </div>
        ...
        <div class="drawer-footer">
          ...
          <button
            class="btn-save"
            @click="submitForm"
            :disabled="!form.document_name || saving"
          >
...
@font-face {
  ...
  src: url('../fonts/NotoSerifLao-Regular.ttf') format('truetype'); /* ← แก้ชื่อไฟล์ให้ตรง */
  font-weight: 400;
}
@font-face {
  font-family: 'Noto Serif Lao';
...
.doc-table { width: 100%; border-collapse: collapse; font-size: 12.5px; min-width: 600px; }
