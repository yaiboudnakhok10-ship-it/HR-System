<script setup>
import { incidenttitle } from '../stores/incident.stores'
import { document as documentStore } from '../stores/document.stores'
import { onMounted, ref, computed } from 'vue'

const store     = incidenttitle()
...
function resetForm() {
  form.value = { title_name: '', detail: '', document_type_id: null }
  editMode.value = false
  editId.value   = null
}

async function submitForm() {
  if (!form.value.title_name) return
  saving.value = true
  try {
    if (editMode.value && editId.value) {
      await store.updateIncident(editId.value, form.value)
    } else {
      await store.addIncident(form.value)
    }
    closeDrawer()
    store.getIncident()
  } catch (e) {
...
async function confirmDelete() {
  ...
  deleteError.value = ''
  try {
    await store.deleteIncident(deleteTarget.value.id)
    store.getIncident()
    closeDeleteConfirm()
  } catch (e) {
...
const filteredIncidents = computed(() => {
  if (!searchQuery.value.trim()) return store.incident_title
  const q = searchQuery.value.toLowerCase()
  return store.incident_title.filter(item =>
    (item.title_name                      || '').toLowerCase().includes(q) ||
    (item.detail                          || '').toLowerCase().includes(q) ||
    (item.document_type?.document_name    || '').toLowerCase().includes(q) ||
  ...
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <h1 class="page-title">หัวข้อประเภทใบเตือน</h1>
          <p class="page-subtitle">จัดการข้อมูลหัวข้อประเภทใบเตือนทั้งหมดในระบบ</p>
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div class="card-toolbar">
        <div class="toolbar-left">
          <div class="search-box">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="ค้นหา หัวข้อ, ประเภท, รายละเอียด..."
              class="search-input"
            />
          ...
          <transition name="fade">
            <div class="result-chip" v-if="searchQuery">
              ...
              พบ <strong>{{ filteredIncidents.length }}</strong> รายการ
            </div>
          </transition>
        </div>
        <div class="toolbar-right">
          <div class="count-chip">
            ...
            <span>ทั้งหมด <strong>{{ store.incident_title.length }}</strong> รายการ</span>
          </div>
        </div>
      </div>
            ...
            <tr v-else-if="filteredIncidents.length === 0">
              <td colspan="7" class="state-cell">
                <div class="state-content empty">
                  ...
                  <span class="empty-title">{{ searchQuery ? 'ไม่พบข้อมูลที่ค้นหา' : 'ไม่พบข้อมูลหัวข้อ' }}</span>
                  ...
                  <button v-else class="btn-add-empty" @click="openDrawer">
                    <i class="fa fa-plus"></i> เพิ่มหัวข้อรายการแรก
                  </button>
                </div>
              </td>
            </tr>

            <tr v-else v-for="(item, idx) in filteredIncidents" :key="item.id" class="data-row">
              <td class="td-num">{{ idx + 1 }}</td>
              <td>
                <span class="badge-doctype" v-if="item.document_type?.document_name">
              ...
              <td>
                <div class="cell-name">
                  ...
                  <span class="name-text">{{ item.title_name || '—' }}</span>
                </div>
              </td>
      ...
      <!-- Footer -->
      <div class="card-footer" v-if="!store.loading && filteredIncidents.length > 0">
      ...
      <div class="modal-overlay" v-if="deleteConfirmOpen" @click="closeDeleteConfirm">
        <transition name="modal-pop">
          <div class="modal-box" v-if="deleteConfirmOpen" @click.stop>
            ...
            <p class="modal-desc">
              คุณต้องการลบหัวข้อ<br/>
              <strong>{{ deleteTarget?.title_name }}</strong> ใช่หรือไม่?<br/>
      ...
      <div class="drawer-panel" v-if="drawerOpen">
        ...
        <div class="drawer-header">
          <div class="drawer-header-left">
            ...
            <div>
              <h2 class="drawer-title">{{ editMode ? 'แก้ไขหัวข้อ' : 'เพิ่มหัวข้อใหม่' }}</h2>
        ...
        <div class="drawer-body">
          ...
          <div class="form-section">
            <div class="form-section-label">
              <i class="fa fa-file-circle-exclamation"></i> ข้อมูลหัวข้อ
            </div>

            <!-- Dropdown ประเภทใบเตือน -->
            <div class="form-group">
              <label class="form-label required">ประเภทใบเตือน</label>
              <div class="select-wrap">
                <select v-model="form.document_type_id" class="form-select">
                  <option :value="null" disabled>-- เลือกประเภทใบเตือน --</option>
                  <option
                    v-for="doc in docStore.document_type"
                    :key="doc.id"
                    :value="doc.id"
                  >
                    {{ doc.document_name }}
            ...
            <!-- ชื่อหัวข้อ -->
            <div class="form-group">
              <label class="form-label required">ชื่อหัวข้อ</label>
              <input
        ...
        <div class="drawer-footer">
          ...
          <button
            class="btn-save"
            @click="submitForm"
            :disabled="!form.title_name || !form.document_type_id || saving"
          >
...
.incident-table { width: 100%; border-collapse: collapse; font-size: 12.5px; min-width: 700px; }
.incident-table thead tr { background: var(--sb-search-bg); border-bottom: 2px solid var(--tb-border); }
.incident-table th { padding: 11px 14px; text-align: left; font-size: 11px; font-weight: 800; color: var(--tb-sub); text-transform: uppercase; letter-spacing: 0.6px; white-space: nowrap; }
.th-num { width: 44px; text-align: center; }
.incident-table tbody tr { border-bottom: 1px solid var(--tb-border); transition: background 0.15s; }
.incident-table tbody tr:last-child { border-bottom: none; }
.data-row:hover { background: var(--sb-hover); }
.incident-table td { padding: 11px 14px; color: var(--tb-text); vertical-align: middle; }
.td-num { text-align: center; font-size: 11.5px; font-weight: 700; color: var(--tb-sub); }
.badge-doctype { display: inline-flex; align-items: center; gap: 5px; padding: 3px 9px; background: rgba(25,118,210,0.08); border: 1px solid rgba(25,118,210,0.18); border-radius: 6px; font-size: 11.5px; font-weight: 700; color: var(--accent); white-space: nowrap; }
.badge-doctype i { font-size: 10px; opacity: 0.7; }
...
.form-select:focus { border-color: var(--accent); background: var(--tb-bg); box-shadow: 0 0 0 3px var(--accent-glow2); }
.select-icon { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); font-size: 11px; color: var(--tb-sub); pointer-events: none; }
